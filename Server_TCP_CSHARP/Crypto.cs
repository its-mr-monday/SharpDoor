...
namespace SharpDoor_Server
{
	class Crypto
	{
        ...
        public static void SendPubKey(NetworkStream stream, TcpClient client, string pubKey)      //Send a message to the target
        {
            ...
            if (message == "PUB KEY RECEIVED")
            {
                Console.WriteLine("client: "+message);
                return;
            }
        }
        public static string ReceivePubKey(NetworkStream stream, TcpClient client)   //Receive a message from the target
        {
            byte[] buffer = new byte[client.ReceiveBufferSize];
            int bytesRead = stream.Read(buffer, 0, client.ReceiveBufferSize);
            string message = Encoding.ASCII.GetString(buffer, 0, bytesRead);
